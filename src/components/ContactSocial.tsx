  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ ...form, subject: 'Homepage Inquiry' }),
      });
      if (res.ok) {
        setSent(true);
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-12'>
          ...
          >
            ...
            <div className='bg-white border-4 border-black rounded-2xl p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col gap-5'>
              ...
              {sent ? (
                <div className='bg-[#e8f4ff] border-2 border-[#1a84d2] rounded-xl p-6 flex flex-col items-center gap-3 text-center'>
                  <span className='material-symbols-outlined text-4xl text-[#1a84d2]' style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
                  <p className='font-black text-[#1a84d2] text-sm uppercase tracking-wide' style={{ fontFamily: 'var(--font-space-grotesk)' }}>Message Sent!</p>
                  <p className='font-bold text-xs text-[#1a84d2]' style={{ fontFamily: 'var(--font-manrope)' }}>We&apos;ll get back to you within 24 hours.</p>
                  <button onClick={() => setSent(false)} className='text-xs font-black underline text-[#1a84d2] mt-1'>Send another</button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
                  ...
                  {error && (
                    <p className='text-xs font-bold text-red-600' style={{ fontFamily: 'var(--font-manrope)' }}>{error}</p>
                  )}
                  ...
                  >
                    {loading ? 'Sending...' : (
                      <>Send Message <span className='material-symbols-outlined text-xl'>send</span></>
                    )}
                  </button>
                </form>
              )}
